...
-- when window config is reloaded, check if dark mode is active
-- and set command palette background accordingly
wezterm.on("window-config-reloaded", function(window)
  local overrides = window:get_config_overrides() or {}
  local appearance = window:get_appearance()
  if appearance:find "Dark" then
    overrides.command_palette_bg_color = nil
    window:set_config_overrides(overrides)
  else
    overrides.command_palette_bg_color = "rgba(65,86,129, 0.85)"
    window:set_config_overrides(overrides)
  end
end)
...
-- wezterm.on("send-status-msg", function(window, msg)
--   my_helpers.send_status_msg(window, msg)
-- end)
...
return {
  ...
  command_palette_bg_color = "rgba(19, 33, 69, 0.95)",
  ...
  --[[ New! (20230326-111934-3666303c) Background Blur
      Undecided if I like or not ]]
